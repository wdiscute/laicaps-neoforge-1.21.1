...
public class TelescopeMenu extends AbstractContainerMenu
{
    ...
    public TelescopeMenu(int containerId, Inventory inv, BlockEntity blockEntity)
    {
        ...
        for (int i = 0; i < 3; ++i)
        {
            for (int l = 0; l < 9; ++l)
            {
                this.addSlot(new Slot(inv, l + i * 9 + 9, (8 + l * 18) - 170, 83 + i * 18));
            }
        }
        ...
        for (int i = 0; i < 9; ++i)
        {
            this.addSlot(new Slot(inv, i, (8 + i * 18)  - 170, 140));
        }


        this.addSlot(new SlotItemHandler(this.blockEntity.inventory, 0, -90, 34));
    }
